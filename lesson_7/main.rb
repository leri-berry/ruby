...
class Main
  ...
  def create_train
    ...
    if train == 1
      ...
      number = gets.chomp
      train = PassengerTrain.new(name, number)
    elsif train == 2
      puts 'Введите имя поезда'
      name = gets.chomp
      puts 'Введите номер поезда'
      number = gets.chomp
      train = CargoTrain.new(name, number)
    end
    @trains << train
    puts "Поезд #{train.number} создан" if train.valid?
  rescue RuntimeError => e
    puts "Поезд #{number} не создан , #{e.message}"
    retry
  end
  ...
  def add_carriage
    ...
    if train.type == 'cargo'
      ...
      carriage = CargoCarriage.new(number, capacity)
    elsif train.type == 'passenger'
      puts 'Укажите номер вагона'
      number = gets.chomp.to_i
      puts 'Укажите кол-во мест'
      number_seats = gets.chomp.to_i
      carriage = PassengerCarriage.new(number, number_seats)
    end
  ...
  def see_trains_carriages
    @trains.each { |train| train.list_carriages }
  end

  def see_station_trains
    @stations.each { |station| station.list_trains }
  end
